namespace GleyMobileAds
{
    ...
    {
#if USE_ADMOB
        public static CustomAdmob Instance { set; get; }
        ...
        private void LoadBanner(BannerPosition position, BannerType bannerType)
        {
            ...
            switch (position)
            {
                ...
                case BannerPosition.LEFT:
                    if (bannerType == BannerType.SmartBanner)
                    {
                        banner = new BannerView(bannerId, AdSize.SmartBanner, AdPosition.TopRight);
                    }
                    else
                    {
                        if (bannerType == BannerType.Adaptive)
                        {
                            AdSize adaptiveSize = AdSize.GetCurrentOrientationAnchoredAdaptiveBannerAdSizeWithWidth(AdSize.FullWidth);
                            banner = new BannerView(bannerId, adaptiveSize, AdPosition.TopRight);
                        }
                        else
                        {
                            banner = new BannerView(bannerId, AdSize.Banner, AdPosition.TopRight);
                        }
                    }
                    break;
            }
